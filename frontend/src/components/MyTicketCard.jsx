...
import { timeAgo, ticketId } from '../utils/incidents'
...
export default function MyTicketCard({ incident, onOpen }) {
  ...
  return (
    <Card elevation={0} sx={(t) => ({ borderRadius: 1.25, boxShadow: t.palette.fixline.shadow, border: '1px solid transparent', '&:hover': { borderColor: alpha(t.palette.primary.main, 0.4) } })}>
      <CardActionArea id={`my-ticket-${incident.id}`} onClick={() => onOpen(incident.id)} sx={{ p: 2 }}>
        <Stack spacing={1.25}>
          <Stack direction="row" sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
            <Typography sx={{ fontFamily: fonts.data, fontSize: 12, color: 'text.disabled' }}>{ticketId(incident.id)} · {timeAgo(incident.createdAt)} ago</Typography>
